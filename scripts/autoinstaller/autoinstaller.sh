...
MONGOC_DRIVER_RESTORE_VERSION="mongo-c-driver-1.17.1"
...
SYSTEMD_SERVICE_FILE_XCASH_PAYOUTS=""
...
FIREWALL_SHARED_DELEGATES_URL="https://raw.githubusercontent.com/Xcash-Labs/xcash-labs-dpops/master/scripts/firewall/firewall_shared_delegates_script.sh"
FIREWALL_XCASH_NODE_URL="https://raw.githubusercontent.com/Xcash-Labs/xcash-labs-dpops/master/scripts/firewall/firewall_script_xcash_node.sh"
...
SYSTEMD_SERVICE_FILE_XCASH_PAYOUTS=$(cat <(curl -sSL $SYSTEMD_SERVICE_FILE_XCASH_PAYOUTS_URL))
SYSTEMD_SERVICE_FILE_XCASH_PAYOUTS="${SYSTEMD_SERVICE_FILE_XCASH_PAYOUTS//'${USER}'/$USER}"
SYSTEMD_SERVICE_FILE_XCASH_PAYOUTS="${SYSTEMD_SERVICE_FILE_XCASH_PAYOUTS//'${XCASH_PAYOUTS_DIR}'/$XCASH_PAYOUTS_DIR}"
...
function get_current_xcash_wallet_data()
{
  ...
  screen -dmS XCASH_RPC_Wallet "${XCASH_DIR}"build/Linux/master/release/bin/xcash-wallet-rpc --wallet-file "${XCASH_WALLET_DIR}"delegate-wallet --password "${WALLET_PASSWORD}" --rpc-bind-port 18288 --confirm-external-bind --disable-rpc-login --trusted-daemon --log-file "${XCASH_LOGS_DIR}"xcash-wallet-rpc.log
  sleep 10s
...
function stop_systemd_service_files()
{
  ...
  sleep 1s
  if systemctl list-unit-files --type=service --no-legend | awk '{print $1}' | grep -qx "xcash-payouts.service"; then
    sudo systemctl stop xcash-payouts &>/dev/null
  fi
...
function  enable_service_files_at_startup()
{
  ...
  sudo systemctl enable mongodb.service xcash-daemon.service xcash-dpops.service xcash-rpc-wallet.service &> /dev/null
  if systemctl list-unit-files --type=service --no-legend | awk '{print $1}' | grep -qx "xcash-payouts.service"; then
    sudo systemctl enable xcash-payouts.service &>/dev/null || true
  fi
...
function download_xcash_payouts()
{
  ...
  git clone --quiet ${XCASH_PAYOUTS_URL}
  echo -ne "\r${COLOR_PRINT_GREEN}Downloading xcash-payouts${END_COLOR_PRINT}"
  echo
}

function build_xcash_payouts()
{
  echo -ne "${COLOR_PRINT_YELLOW}Building xcash-payouts${END_COLOR_PRINT}"
  cd "${XCASH_PAYOUTS_DIR}"
  git checkout --quiet ${XCASH_PAYOUTS_BRANCH}
  if [ "$RAM_CPU_RATIO" -ge "$RAM_CPU_RATIO_ALL_CPU_THREADS" ]; then
    make clean &>/dev/null
    make release -j "${CPU_THREADS}" &>/dev/null
  else
  ...
  echo -ne "\r${COLOR_PRINT_GREEN}Building xcash-payouts${END_COLOR_PRINT}"
  echo
}
...
function create_xcash_wallet()
{
  ...
  echo "exit" | "${XCASH_DIR}"build/Linux/master/release/bin/xcash-wallet-cli --generate-new-wallet "${XCASH_DPOPS_INSTALLATION_DIR}"xcash-wallets/delegate-wallet --password "${WALLET_PASSWORD}" --mnemonic-language English --restore-height 0 --trusted-daemon | stdbuf -oL tr '\r' '\n' | stdbuf -o 0 grep -C 1 "Height" | stdbuf -o 0 awk '{print "Processing: ",$1,$2,$3,$4}' ORS="\r"
  echo -ne "                                                                              \r"
...
function import_xcash_wallet()
{
  ...
  echo -e "${COLOR_PRINT_GREEN}Starting Wallet Refresh${END_COLOR_PRINT}"
  (echo -ne "\n"; echo "${WALLET_PASSWORD}"; echo "exit") | "${XCASH_DIR}"build/Linux/master/release/bin/xcash-wallet-cli --restore-deterministic-wallet --electrum-seed "${WALLET_SEED}" --generate-new-wallet "${XCASH_DPOPS_INSTALLATION_DIR}"xcash-wallets/delegate-wallet --password "${WALLET_PASSWORD}" --mnemonic-language English --restore-height 0 --trusted-daemon | stdbuf -oL tr '\r' '\n' | stdbuf -o 0 grep -C 1 "Height" | stdbuf -o 0 awk '{print "Processing: ",$1,$2,$3,$4}' ORS="\r"
  echo -ne "                                                                              \r"
  echo -e "${COLOR_PRINT_GREEN}Wallet Refresh Completed${END_COLOR_PRINT}"
...
function install_xcash_payouts()
{
  ...
  echo
  echo -e "${COLOR_PRINT_GREEN}############################################################${END_COLOR_PRINT}"
  echo -e "${COLOR_PRINT_GREEN}                Installing xcash-payouts${END_COLOR_PRINT}"
  echo -e "${COLOR_PRINT_GREEN}############################################################${END_COLOR_PRINT}"
  download_xcash_payouts
  build_xcash_payouts
